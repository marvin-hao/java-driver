...
public class TableOptionsIT {
  ...
  public void ensure_table_options() {
    ...
    Optional<? extends TableMetadata> tableMeta = ks.get().getTable("foo");
    Map<CqlIdentifier, Object> tableOptions = tableMeta.get().getOptions();
    assertThat(tableOptions.containsKey(readRepairKey)).isTrue();
    assertThat(tableOptions.containsKey(additionalWritePolicyKey)).isTrue();
    assertThat(tableOptions.get(readRepairKey)).isEqualTo("NONE");
    assertThat(tableOptions.get(additionalWritePolicyKey)).isEqualTo("40p");
  }
}
