...
class InitialNodeListRefresh extends NodesRefresh {
  ...
  @VisibleForTesting final Set<Node> contactPoints;

  InitialNodeListRefresh(Iterable<NodeInfo> nodeInfos, Set<Node> contactPoints) {
  ...
  public Result compute(
    ...
    ImmutableMap<UUID, Node> newNodes = newNodesBuilder.build();
    ...
    for (Node newNode : newNodes.values()) {
      if (!contactPoints.contains(newNode)) {
        eventsBuilder.add(NodeStateEvent.added((DefaultNode) newNode));
      }
    }
    for (Node contactPoint : contactPoints) {
      if (findIn(newNodes.values(), contactPoint.getEndPoint()) == null) {
        eventsBuilder.add(NodeStateEvent.removed((DefaultNode) contactPoint));
      }
    }

    return new Result(
        oldMetadata.withNodes(newNodes, tokenMapEnabled, tokensChanged, tokenFactory, context),
        eventsBuilder.build());
  }

  private DefaultNode findIn(Iterable<Node> nodes, EndPoint endPoint) {
    for (Node node : nodes) {
